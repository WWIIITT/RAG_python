...
def _split_docs_into_chunks(docs: List[Dict[str, Any]], renumber_page_by_chunk: bool = False) -> List[Dict[str, Any]]:
    ...
    for d in docs:
        ...
        if renumber_page_by_chunk:
            for idx, chunk_text in enumerate(parts):
                chunks.append({
                    "pageContent": chunk_text,
                    "metadata": {**d.get("metadata", {}), "pageNumber": idx + 1},
                })
        else:
...
async def _extract_graph_with_progress(
    chunks: List[Dict[str, Any]],
    ...
    async def extract_one(idx: int, chunk: Dict[str, Any]) -> None:
        ...
        print(f"[Graph] [Start] 任務 {idx + 1}/{len(chunks)} (頁 {page_no}) len={len(chunk_text)}")
        attempts = 0
        backoff_base = 1.0
        while attempts < MAX_RETRIES_PER_TASK:
            ...
            try:
                ...
                async with _completed_lock:
                    _completed += 1
                    print(f"[Graph] [OK] 任務 {idx + 1}/{len(chunks)} (頁 {page_no}) -> 完成 {_completed}/{len(chunks)}")
                ...
                if retry_delay_match:
                    try:
                        ...
                        pass
                print(f"[Graph] [Retry] 任務 {idx + 1}/{len(chunks)} (頁 {page_no}) 第 {attempts + 1} 次，等待 {delay_seconds:.1f}s; 錯誤: {err}")
                if not switch_to_next_key():
                    attempts += 1
                await asyncio.sleep(delay_seconds)

        all_extractions[idx] = {"entities": [], "relationships": []}
        print(f"[Graph] [Fail] 任務 {idx + 1}/{len(chunks)} (頁 {page_no})，已達最大重試")
        async with _completed_lock:
            _completed += 1
            await publish_progress(client_id, {"type": "progress", "done": _completed, "total": total_with_db})
...
def _assemble_chunks_with_graph(
    chunks: List[Dict[str, Any]],
    all_vectors: List[List[float]],
...
async def process_website_and_create_graph(*, url: str, client_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    parsed = urlparse(url)
    last_segment = (parsed.path or "").strip("/")
    last_segment = last_segment.split("/")[-1] if last_segment else ""
